#!/usr/bin/env python3
"""
Premium Morning Briefing Generator
...
class PremiumMorningBriefing:
    ...
    def format_for_briefing(self, all_data: Dict[str, Any], selected_stories: Dict[str, List[Dict]]) -> str:
        ...
        if indices and indices.get("premarket"):
            briefing_parts.append("Index Futures:")
            for item in indices["premarket"]:
                symbol = item.get("symbol", "")
                price = item.get("preMarketPrice")
                change = item.get("preMarketChange")
                change_pct = item.get("preMarketChangePercent")
                prev_close = item.get("lastClose")
                bid = item.get("bid")
                ask = item.get("ask")
                ...
                if price is not None:
                    ...
                    if change is not None and change_pct is not None:
                        ...
                        briefing_parts.append(f"  • {symbol}: ${price:.2f} ({direction}{change:.2f}, {direction}{change_pct:.2f}%)")
                        if prev_close:
                            briefing_parts.append(f"    Previous close: ${prev_close:.2f}")
                    # Fallback to bid/ask if no change data
                    elif bid and ask:
                        briefing_parts.append(f"  • {symbol}: ${price:.2f} (bid: ${bid:.2f}, ask: ${ask:.2f})")
                    else:
                        briefing_parts.append(f"  • {symbol}: ${price:.2f}")
                else:
                    briefing_parts.append(f"  • {symbol}: No data available")
    ...
    async def summarize_and_select_articles(self, all_data: Dict[str, Any], selected_stories: Dict[str, List[Dict]]) -> str:
        ...
        # Extract premarket indices data
        indices = premarket.get("indices", {})
        indices_summary = indices.get("summary", "") if indices else ""
        
        # Format premarket indices for prompt
        premarket_text = ""
        if indices and indices.get("premarket"):
            premarket_text = "PREMARKET INDEX FUTURES:\n"
            for item in indices["premarket"]:
                symbol = item.get("symbol", "")
                price = item.get("preMarketPrice")
                change = item.get("preMarketChange")
                ...
                display_symbol = "DJI (via DIA)" if symbol == "DIA" else symbol
                
                if price is not None and change is not None and change_pct is not None:
                    direction = "up" if change >= 0 else "down"
                    premarket_text += f"- {display_symbol}: ${price:.2f} (previous close: ${prev_close:.2f}, {direction} ${abs(change):.2f} or {abs(change_pct):.2f}%)\n"
        elif indices_summary:
            premarket_text = f"PREMARKET STATUS: {indices_summary}\n"
        ...
        TASK: Analyze the following {len(all_articles)} news articles and create a comprehensive 12-15 minute professional morning briefing.
        ...
        3. CREATE a 12-15 minute briefing (MINIMUM 1800 words, TARGET 2000-2250 words) with this EXACT structure:

           START WITH: "Welcome to your MarketMotion Daily briefing. It's [date]. [Include premarket futures data if provided - mention SPY, QQQ, DJI movement]. [crypto overview if relevant - use periods to separate crypto prices, not pipes]"
           Example opening: "Welcome to your MarketMotion Daily briefing. It's August eighteenth. Index futures are mixed this morning, with SPY down zero point three percent, QQQ down zero point five percent, and the Dow slightly lower. Bitcoin is at one hundred fifteen thousand dollars, down one point nine percent. Ethereum is at four thousand two hundred fifty nine dollars, down four point eight percent."

           Then say "Top stories..." and provide 4-5 most critical developments (450 words total)

           Then say "Market movers..." and cover key financial/economic news (400 words)

           Then say "Global developments..." and cover international news (400 words)

           Then say "Technology and innovation..." and cover tech news (350 words)

           Then say "Sector highlights..." and cover other sectors (300 words)

           Then say "Economic calendar for the week ahead..." and briefly mention the HIGH-IMPACT economic events for the next 3 business days (200 words)
        ...
        - MINIMUM 1800 words total (12 minutes at 150 wpm)
        - TARGET 2000-2250 words total (13-15 minutes at 150 wpm)
        - Each section MUST hit its word count target - no exceptions
        - If you write less than 1800 words, you FAILED the assignment
        - Better to be comprehensive than brief - investors want detailed analysis
...
async def main():
    ...
    generator = PremiumMorningBriefing()
    ...
    print(f"📝 Gemini briefing: {result['gemini_word_count']} words (12-15 minute target: 1800-2250 words)")
